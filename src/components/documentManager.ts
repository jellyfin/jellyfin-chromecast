...
export abstract class DocumentManager {
   ...
   */
  private static getPrimaryImageUrl(item: BaseItemDto): Promise<string | null> {
  ...
  private static getLogoUrl(item: BaseItemDto): Promise<string | null> {
    ...
    if (item.ImageTags?.Logo && item.Id) {
      src = JellyfinApi.createImageUrl(item.Id, 'Logo', item.ImageTags.Logo);
  ...
  private static setPlayedIndicator(value: boolean | number): void {
    ...
    const playedIndicatorValue = this.getElementById('played-indicator-value');
  ...
  private static setRating(item: BaseItemDto): void {
    ...
    if (item.CriticRating != null) {
      ...
      criticRating.classList.remove(verdict == 'fresh' ? 'rotten' : 'fresh');
  ...
  public static getWaitingBackdropUrl(
    ...
    if (item != null) {
      if (item.BackdropImageTags && item.BackdropImageTags.length && item.Id) {
  ...
  private static setPlayedPercentage(value = 0): void {
    const element = <HTMLInputElement>this.querySelector('.itemProgressBar');
  ...
  private static setMiscInfo(item: BaseItemDto): void {
    ...
    if (item.Type == 'Episode') {
      if (item.PremiereDate) {
        try {
          info.push(parseISO8601Date(item.PremiereDate).toLocaleDateString());
        } catch (e) {
    ...
    if (item.StartDate) {
      try {
        info.push(parseISO8601Date(item.StartDate).toLocaleDateString());
      } catch (e) {
        console.log(`Error parsing date: ${item.PremiereDate}`);
      }
    }
    ...
    if (item.ProductionYear && item.Type == 'Series') {
      if (item.Status == 'Continuing') {
        ...
        if (item.EndDate) {
          try {
            const endYear = parseISO8601Date(item.EndDate).getFullYear();
